...
; Entity structure offsets - MUST MATCH hal/entity.h!
; See PROJECT_ARCHITECTURE.md for field usage.
;
...
; Total: 16 bytes
;
ENT_FLAGS       = 0             ; 1 byte - flags (offset 0 per HAL spec)
ENT_TYPE        = 1             ; 1 byte - entity type
...
ENT_VX_LO       = 6             ; 1 byte - X velocity (fractional)
ENT_VX_HI       = 7             ; 1 byte - X velocity (whole/signed)
ENT_VY_LO       = 8             ; 1 byte - Y velocity (fractional)
ENT_VY_HI       = 9             ; 1 byte - Y velocity (whole/signed)
ENT_HP          = 10            ; 1 byte - Health
ENT_TIMER       = 11            ; 1 byte - General timer
ENT_SPRITE      = 12            ; 1 byte - Sprite asset ID
ENT_FRAME       = 13            ; 1 byte - Animation frame
ENT_DATA_LO     = 14            ; 1 byte - Type-specific data low
ENT_DATA_HI     = 15            ; 1 byte - Type-specific data high

; Legacy aliases for backward compatibility
ENT_VX          = ENT_VX_LO     ; Single-byte velocity (use lo byte)
ENT_VY          = ENT_VY_LO
ENT_SUBTYPE     = ENT_DATA_LO   ; Subtype stored in data.lo
ENT_ANIM_FRAME  = ENT_FRAME
